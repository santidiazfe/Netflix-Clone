...
const Pelicula = () => {
  ...
  const getSimilar = async () => {
    await fetch(`https://api.themoviedb.org/3/tv/${id}/similar?api_key=35ae0ab849a7893db02cf33f003e017f`)
  ...
  const getTrailer = async () => {
    ...
    await fetch(`https://api.themoviedb.org/3/tv/${id}/videos?api_key=35ae0ab849a7893db02cf33f003e017f`)
    .then(response => response.json())  
    .then((trailer) => {
        setTrailer(trailer)
      })
  ...
  const getSerie= async () => {
    await fetch(`https://api.themoviedb.org/3/tv/${id}?api_key=35ae0ab849a7893db02cf33f003e017f`)
    .then(response => response.json())  
    .then((serie) => {
        setSerie(serie)
        setGeneros(serie.genres)
    ...
    <div className='bg-black text-white md:px-20 md:pt-2 w-full '>
        ...
        <div className=' ml-4 text-white mt-6'>
          <div className='flex justify-between items-center'>
            <p className='text-sm font-semibold text-zinc-300 '>{serie.first_air_date}</p>
           
          </div>  
          <div className='md:flex'>
            <div className='md:w-1/2 flex flex-col md:mt-10 '>     
              <h3 className='text-2xl font-bold mb-2 ' >{serie.original_name}</h3>
              <p className='mt-4 w-[80%]'>{serie.overview}</p>
